...
class LoadBalancerServiceIT extends AbstractPostgresIT {
...
//    }
    ...
    void registerDuplicateIsRejected_perTenantAndGroup() {
        loadBalancer.register(SCOPE1, " http://a ");

        assertThatThrownBy(() -> loadBalancer.register(SCOPE1, "http://a"))
                .isInstanceOf(DuplicateAddressException.class);

        // same address is allowed in different group or tenant (by design)
        assertThatCode(() -> loadBalancer.register(SCOPE2, "http://a")).doesNotThrowAnyException();
        assertThatCode(() -> loadBalancer.register(SCOPE3, "http://a")).doesNotThrowAnyException();
    }

    @Test
    void capacityUpTo10_isPerTenantAndGroup() {
        for (int i = 0; i < 10; i++) {
            loadBalancer.register(SCOPE1, "http://n" + i);
        }

        assertThatThrownBy(() -> loadBalancer.register(SCOPE1, "http://overflow"))
                .isInstanceOf(CapacityExceededException.class);

        // another group has its own capacity bucket
        assertThatCode(() -> loadBalancer.register(SCOPE2, "http://overflow"))
    ...
    void roundRobinNext_isPerTenantAndGroup() {
        loadBalancer.register(SCOPE1, "http://a");
        loadBalancer.register(SCOPE1, "http://b");
        loadBalancer.register(SCOPE1, "http://c");

        assertThat(loadBalancer.nextAddress(SCOPE1)).contains("http://a");
        assertThat(loadBalancer.nextAddress(SCOPE1)).contains("http://b");
        assertThat(loadBalancer.nextAddress(SCOPE1)).contains("http://c");
        assertThat(loadBalancer.nextAddress(SCOPE1)).contains("http://a");

        // different group is independent
        loadBalancer.register(SCOPE2, "http://s1");
        assertThat(loadBalancer.nextAddress(SCOPE2)).contains("http://s1");
    }
}
